            ...
            if (sess.logado == 1) {
                ...
                async function prereq(dados) {
                    ...
                    if (typeof result != 'undefined') {
                        ...
                        if (prereq.wskStatus == null || prereq.wskStatus != 0){
                            var usuario = await buscaUsuario(user);
                            var permissao = await permissaoUsuario(user);
                            var error = 1;
                            res.render('index.ejs', {
                ...
                async function insertKmtbParts(dados){
                    var result = await priceBanco.insertKmtbParts(dados);
                ...
                async function insere_sekUF(dados) {
                    var result = await priceBanco.insere_sekUF(dados);
                ...
                async function renderizar() {
                    ...
                    var teste = await selectPartsPrice(dados);
                    if (peca != 0 && peca !=1){
                        if (typeof peca.peca == 'string'){
                            if (peca.sekUF < 16){
                                insere_sekUF(peca.peca);
                                peca = await prereq(dados);
                                insertKmtbParts(dados);
                                res.render('index.ejs', {
                                    'dados': usuario,
                                    'permissao': permissao,
                                    'peca': peca,
